...
/**
 * 懒加载fragment基类（结合ViewPager使用）
 * @author 周俊蒙
 * @version 1.0.0
 * @date 2019-6-12
 * @since JDK 1.8.0_191
 */
abstract class ViewPagerLazyFragment<VB : ViewDataBinding, VM : BaseViewModel> : Fragment() {
    ...
    fun presenter() {
        mModel.mPresenter.observe(this, Observer {
            ...
            if (it.login) {
                // 需要登录时跳转登录界面
                // TODO:因为Baselib不依赖provider模块，因此此处地址用写死的LoginActivity的路径字符串，不能用ARouterPath
                ARouter.getInstance()
