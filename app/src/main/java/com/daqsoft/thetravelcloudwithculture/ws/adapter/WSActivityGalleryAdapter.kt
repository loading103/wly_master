 ...
 */
class WSActivityGalleryAdapter : SCPopularActivitiesStyleBaseAdapter<WSActivityGalleryAdapter.RecyclerViewItemHolder>() {

    inner class RecyclerViewItemHolder(val binding: ItemHomePopularActivityScStyleThreeBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerViewItemHolder {
        val mBinding = DataBindingUtil.inflate<ItemHomePopularActivityScStyleThreeBinding>(
            LayoutInflater.from(parent.context),
            com.daqsoft.provider.R.layout.item_home_popular_activity_sc_style_three,
            parent,
            false
        )
    ...
    private fun bindDataToView(mBinding: ItemHomePopularActivityScStyleThreeBinding, position: Int) {
        ...
        mBinding.root.onModuleNoDoubleClick(
            BaseApplication.context.resources.getString(com.daqsoft.provider.R.string.provider_hot_activity),
...
//                                    .build( MainARouterPath.MAIN_SEAT_SELECT_ACTIVITY)
        ...
        if (item.type == ActivityType.ACTIVITY_TYPE_PLAIN) {
            ...
            if (item.money.toDouble() == 0.0 && item.integral == "0") {
                mBinding.unit.visibility = View.GONE
                mBinding.price.text = mBinding.price.context.getString(com.daqsoft.provider.R.string.provider_order_free)
        ...
        if (!item.useStartTime.isNullOrEmpty() && !item.useEndTime.isNullOrEmpty()) {
            ...
            // 左上角时间
            val time = DateUtil.formatDateByString(
                "yyyy-MM-dd",
                "yyyy-MM-dd HH:mm:ss",
                item.useStartTime
            )
            mBinding.year.text = time.split("-")[0]
            mBinding.month.text = Utils.coverMonthAbbreviation(time.split("-")[1])
            mBinding.day.text = time.split("-")[2]
        }
        ...
        when (item.type) {
            ...
            ActivityType.ACTIVITY_TYPE_ENROLL -> {
                ...
                if (!item.signStartTime.isNullOrEmpty() && !item.signEndTime.isNullOrEmpty()) {
                    ...
                    // 左上角时间
                    mBinding.month.text = Utils.coverMonthAbbreviation(startTime.split(".")[0])
                    mBinding.day.text = startTime.split(".")[1]
                }
            }
...
//                    activityRunName += "·还剩" + activityBean.stock + "个名额"
                    val view = LayoutInflater.from(mBinding.root.context).inflate(
                        com.daqsoft.provider.R.layout.home_popular_activity_flowlayout_item,
                        mBinding.flActivityLabel,
                        false
                    )
                    val label = view.findViewById<TextView>(com.daqsoft.provider.R.id.label)
                    label.text = "还剩${item.stock}个名额"
                    label.textColorResource = com.daqsoft.provider.R.color.ff9e05
                    label.backgroundResource = com.daqsoft.provider.R.drawable.shape_home_popular_activity_flowlayout_item_bg_orange
        ...
        val view = LayoutInflater.from(mBinding.root.context).inflate(
            com.daqsoft.provider.R.layout.home_popular_activity_flowlayout_item_style_three,
            mBinding.flActivityLabel,
            false
        )
        view.findViewById<TextView>(com.daqsoft.provider.R.id.label).text = activityRunName
        mBinding.flActivityLabel.addView(view)
        ...
        tags.forEach {
            ...
            view.findViewById<TextView>(com.daqsoft.provider.R.id.label).text = it
            mBinding.flActivityLabel.addView(view)
        }
        ...
        item.tagNames?.takeIf { it.isNotEmpty() }?.let {
            it.split(",").forEachIndexed { index, s ->
                if (index < 3) {
                    ...
                    view.findViewById<TextView>(com.daqsoft.provider.R.id.label).text = s
                    mBinding.flActivityLabel.addView(view)
                }
