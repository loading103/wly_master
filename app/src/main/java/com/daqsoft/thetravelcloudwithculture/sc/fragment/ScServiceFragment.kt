...
import com.daqsoft.thetravelcloudwithculture.databinding.FragmentScServiceBinding
 ...
 */
class ScServiceFragment() : BaseFragment<FragmentScServiceBinding, ServiceViewModel>() {
    ...
    override fun getLayout(): Int = R.layout.fragment_sc_service
            ...
            if (!it.adInfo.isNullOrEmpty()) {
                ...
                RxView.clicks(mBinding.ivAd)
                    ...
                    .throttleFirst(1, TimeUnit.SECONDS).subscribe { o ->
                        run {
                            if (!it.adInfo[0].jumpUrl.isNullOrEmpty()) {
                                ARouter.getInstance()
    ...
    private fun showHealthyQrCode() {
        if (mQrCodeDialog == null) {
            mQrCodeDialog = QrCodeDialog.Builder()
                .qrCodeImageUrl(WebSiteAouter.HEALTH_CODE_URL)
                .title("健康码")
            ...
            mQrCodeDialog?.updateData(WebSiteAouter.HEALTH_CODE_URL, "健康码")
        }
        mQrCodeDialog!!.show()
    }
