...
import com.daqsoft.provider.databinding.ItemFoundTemplateTwoRecyBinding
...
/**
 * @Description
 * @ClassName   FoundTemplateTwoAdapter
 * @Author      luoyi
 * @Time        2020/10/15 17:11
 */
class FoundItemTwoAdapter :
    RecyclerViewAdapter<ItemFoundTemplateTwoRecyBinding, FoundAroundBean>(R.layout.item_found_template_two_recy) {
    var currentPostion: LatLng? = null
    override fun setVariable(
        mBinding: ItemFoundTemplateTwoRecyBinding,
        position: Int,
        item: FoundAroundBean
    ) {
        if (position == 0) {
            mBinding.tvModeNear.visibility = View.VISIBLE
        } else {
            mBinding.tvModeNear.visibility = View.GONE
        }
        ...
        item?.let {
            ...
            spb.append(text)
            mBinding.tvFoundContent.text = spb
