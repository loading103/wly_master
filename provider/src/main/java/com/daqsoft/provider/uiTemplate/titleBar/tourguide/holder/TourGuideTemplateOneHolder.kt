 ...
 */
class TourGuideTemplateOneHolder(binding: ItemTourGuideTemplateOneBinding) :
    BaseOperationViewHolder<ItemTourGuideTemplateOneBinding>(binding) {
...
//        binding
    ...
    fun bindData(tourBean: TourHomeBean?,template: CommonTemlate?) {
        tourBean?.let {
            ...
            binding.tourInfo = DividerTextUtils.convertString(
                StringBuilder(),
                "景点·${it.placeNum} ",
                " 线路·${it.lineNum} ",
                " 停车场·${it.parkingNum} ",
