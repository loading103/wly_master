 ...
 * @since JDK 1.8
 */
class ImageRoomPagerAdapter(imgDatas: MutableList<OrderRoomInfo>, context: Context) : PagerAdapter() {
    /**
     * 图片适配器
     */
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        ...
        var imageUrl: String = ""
        if (!imgDatas.get(index).img.isNullOrEmpty()) {
            var imageUrls = imgDatas.get(index).img.split(",")
