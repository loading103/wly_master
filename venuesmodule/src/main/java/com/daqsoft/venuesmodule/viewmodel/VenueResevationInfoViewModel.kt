...
/**
 * @Description 场馆预订详情
 * @ClassName   VenueResevationInfoViewModel
 * @Author      luoyi
 * @Time        2020/5/12 16:12
 */
class VenueResevationInfoViewModel : BaseViewModel() {
     ...
     * 场馆预约数量
     */
    var venueOrderDateNumLiveData: MutableLiveData<MutableList<VenueDateNumBean>> =
        MutableLiveData()
                ...
                .excute(object : BaseObserver<OrderResultBean>(mPresenter) {
                    override fun onSuccess(response: BaseResponse<OrderResultBean>) {
                        SPUtils.getInstance(SPUtils.SpNameConfig.SPLASH_RESERATION_INFO)
                            .put(SPUtils.Config.APP_RESERATION_NAME, bean.name)
                        SPUtils.getInstance(SPUtils.SpNameConfig.SPLASH_RESERATION_INFO)
                            .put(SPUtils.Config.APP_RESERATION_PHONE, bean.phone)
                        SPUtils.getInstance(SPUtils.SpNameConfig.SPLASH_RESERATION_INFO)
                            .put(SPUtils.Config.APP_RESERATION_IDCARD, bean.idCardNum)
                        if (!bean.cardType.isNullOrEmpty()) {
                            SPUtils.getInstance(SPUtils.SpNameConfig.SPLASH_RESERATION_INFO)
    ...
    fun checkExistNumber(phoneNumber: String) {
        mPresenter?.value?.loading = false
        VenuesRepository.venuesService.getCheckExistOrderNumbers(phoneNumber, "CONTENT_TYPE_VENUE")
            ...
            .excute(object : BaseObserver<OrderSaveBean>(mPresenter) {
                override fun onSuccess(response: BaseResponse<OrderSaveBean>) {
                    ARouter.getInstance()
                        ...
                        .withString("orderType", ResourceType.CONTENT_TYPE_VENUE)
                        .withString("faithAuditStatus", "")
                        .withBoolean("isCommentator", false)
                        .navigation()
        ...
        VenuesRepository.venuesService.getOrderDateList(param)
            .excute(object : BaseObserver<VenueReservationInfo>() {
                ...
                override fun onFailed(response: BaseResponse<VenueReservationInfo>) {
                    venueOrderDateListLiveData.postValue(null)
                }
            })
    }
    ...
    fun getVenueOrderTimes(venueId: String, date: String, type: Int = 1) {
        ...
        VenuesRepository.venuesService.getVenueOrderTimes(venueId, date, typeStr)
            .excute(object : BaseObserver<VenueOrderTime>() {
                ...
                override fun onFailed(response: BaseResponse<VenueOrderTime>) {
                    venueOrderTimesLiveData.postValue(null)
                }
            })
    }
