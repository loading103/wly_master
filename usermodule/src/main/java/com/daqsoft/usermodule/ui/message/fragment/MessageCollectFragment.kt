 ...
 */
class MessageCollectFragment: BaseFragment<FragMessageListBinding, MessageTypeListViewModel>() {
    ...
    override fun initData() {
        classify = (activity as MessageHdListActivity).classify.toString()
        msgLsAdapter?.setLayoutType(classify,type)
        (activity as MessageHdListActivity).showLoadingDialog()
        mModel.getListData(classify,type)
    }

    private fun initViewModel() {
        mModel.datas?.observe(this, Observer {
            (activity as MessageHdListActivity).dissMissLoadingDialog()
