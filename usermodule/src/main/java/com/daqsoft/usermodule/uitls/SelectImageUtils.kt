...
//        val contentView = LayoutInflater.from(activity).inflate(R.layout.popup_picture_select, null)
        val bingding = DataBindingUtil.inflate<PopupPictureSelectBinding>(LayoutInflater.from(activity), R.layout.popup_picture_select, null, false)
            ...
            RxPermissions(activity)
                ...
                .subscribe {
                    if (it) {
                        ...
                        Toast.makeText(activity, R.string.user_msg_no_camera, Toast.LENGTH_SHORT).show()
                    }
                }
        ...
        popUpWindow.setOnDismissListener { Utils.setWindowBackGroud(activity, 1f) }
        return popUpWindow
    }
