...
class SpecialDetailViewModel : BaseViewModel() {
     ...
     * 餐厅详情
     */
    val specialDetailLiveData: MutableLiveData<SpeaiclDetailBean> = MutableLiveData()
    /**
     * 刷新数据状态
     */
    var refreshSpecialDetailLiveData:MutableLiveData<SpeaiclDetailBean> =MutableLiveData()
    /**
     * 收藏状态变更
     */
     ...
     * 美食商品
     */
    var specialProductLiveData = MutableLiveData<MutableList<FoodProductBean>>()
    /**
     * 相关故事列表
     */
    ...
    fun getSpecialDetail(id: String) {
        ...
        MainRepository.service.getSpecialDetail(id).excute(object : BaseObserver<SpeaiclDetailBean>(mPresenter) {
            override fun onSuccess(response: BaseResponse<SpeaiclDetailBean>) {
                specialDetailLiveData.postValue(response.data)
            }

            override fun onFailed(response: BaseResponse<SpeaiclDetailBean>) {
    ...
    fun getScenicContentLs(id: String) {
        ...
        VenuesRepository.venuesService.getContentLs("publishTime", ResourceType.CONTENT_TYPE_SPECIALTY, id)
...
//        mPresenter?.value?.isNeedRecyleView = false
//
//        MainRepository.service.getPlayGroundetail(id).excute(object : BaseObserver<SpeaiclDetailBean>(mPresenter) {
//            override fun onSuccess(response: BaseResponse<SpeaiclDetailBean>) {
//                refreshSpecialDetailLiveData.postValue(response.data)
//            }
//
//            override fun onFailed(response: BaseResponse<SpeaiclDetailBean>) {
//                mError.postValue(response)
...
//        })
    ...
    fun collectionCancel(resourceId: String) {
        mPresenter?.value?.loading = false
        mPresenter?.value?.isNeedRecyleView = false
        CommentRepository.service.posCollectionCancel(resourceId, ResourceType.CONTENT_TYPE_SPECIALTY)
            .excute(object : BaseObserver<Any>(mPresenter) {
                override fun onSuccess(response: BaseResponse<Any>) {
    ...
    fun thumbUp(resourceId: String) {
        mPresenter?.value?.loading = false
        mPresenter?.value?.isNeedRecyleView = false
        CommentRepository.service.postThumbUp(resourceId, ResourceType.CONTENT_TYPE_SPECIALTY)
            .excute(object : BaseObserver<Any>(mPresenter) {
                override fun onSuccess(response: BaseResponse<Any>) {
    ...
    fun thumbCancell(resourceId: String) {
        mPresenter?.value?.loading = false
        mPresenter?.value?.isNeedRecyleView = false
        CommentRepository.service.postThumbCancel(resourceId, ResourceType.CONTENT_TYPE_SPECIALTY)
            .excute(object : BaseObserver<Any>(mPresenter) {
                override fun onSuccess(response: BaseResponse<Any>) {
    ...
    fun getActivityCommentList(id: String) {
        ...
        mPresenter?.value?.isNeedRecyleView = false
        val param = HashMap<String, String>()
        param["resourceType"] = ResourceType.CONTENT_TYPE_SPECIALTY
        param["resourceId"] = id
        param["pageSize"] = "2"
        ...
        ElectronicRepository.electronicService.getFoodProductLs(resourceCode, sysCode)
            .excut(object : ElectronicObserver<MutableList<FoodProductBean>>() {
                override fun onSuccess(response: BaseResponse<MutableList<FoodProductBean>>) {
                    specialProductLiveData.postValue(response.data)
                }
            })
    }
    ...
    fun getOrderAddressInfo(id: String) {
        MainRepository.service.getOrderAddressInfo(ResourceType.CONTENT_TYPE_SPECIALTY, id)
