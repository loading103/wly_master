...
import com.daqsoft.mainmodule.databinding.FragReseachDetialTopBinding
 ...
 * @Time        2020/6/2 11:14
 */
class ScenicDetailTopFragment1 : BaseFragment<FragReseachDetialTopBinding, ScenicDetailViewModel>() {
    /**
     * 景区标签适配器
     */
    ...
    companion object {
        fun newInstance(bean: ResearchDetailBean): ScenicDetailTopFragment1 {
            var frag = ScenicDetailTopFragment1()
            var bundle = Bundle()
    ...
    override fun getLayout(): Int {
        return R.layout.frag_reseach_detial_top
    }
    ...
    private fun initViewEvent() {
        mBinding.tvScenicDetailsAddressTitle.text="基地地址"
    ...
    private fun getParam() {
        var bean = arguments?.getParcelable<ResearchDetailBean>("bean")
        if (bean != null) {
            mBinding.bean = bean
    ...
    private fun bindViewData() {
        mScenicDetailBean?.let {
            ...
            // 标签处理
            if (!it.scenicLevel.isNullOrEmpty() && !it.tag.isNullOrEmpty()) {
                val tags = mutableListOf<ScenicLabelBean>()
                var num = it.scenicLevel!!.count {
                    it == 'A'
                }
                if (num > 0) {
...
//            spanStatus.setSpan(imageSpan, status.length - 3, status.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
            ...
            RxView.clicks(mBinding.tvStatus)
                ...
                .throttleFirst(1, TimeUnit.SECONDS).subscribe {
                    ARouter.getInstance()
                        .build(MainARouterPath.MAIN_RESEARCH_DETAIL_MORE)
