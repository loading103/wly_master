 ...
 */
class HotActivitiesFragmentViewModel : BaseViewModel() {
    /**
     * 当前选中的分类
     */
    ...
    val activityCalenderLd = MutableLiveData<MutableList<ActivityCalenderBean>>()
    /**
     * 选中的地区
     */
    var currentArea = ""
     ...
     * 获取活动日历
     */
    fun getActivityCanlender(time: String) {
        mPresenter?.value?.loading = false
        mPresenter?.value?.isNeedRecyleView = false
        MainRepository.service.getAcivityCalendarLs(time).excute(object : BaseObserver<ActivityCalenderBean>() {
            override fun onSuccess(response: BaseResponse<ActivityCalenderBean>) {
                activityCalenderLd.postValue(response.datas)
            }

            override fun onFailed(response: BaseResponse<ActivityCalenderBean>) {
                activityCalenderLd.postValue(null)
            }
        })
    }
