 ...
 * 全部乡村列表适配器
 */
class CountryAllMoreAdapter : RecyclerViewAdapter<ItemCountryAllListBinding, CountryListBean> {
    ...
    constructor(context: Context) : super(R.layout.item_country_all_list) {
        mContext = context
    }

    @SuppressLint("CheckResult")
    override fun setVariable(
        mBinding: ItemCountryAllListBinding,
        position: Int,
        item: CountryListBean
    ) {
        mBinding.item = item
        // 判断景点是否为空
        if (item.spotsNum>= 2) {
            ...
            val imagePagerAdapter = object : PagerAdapter() {
                ...
                override fun instantiateItem(container: ViewGroup, position: Int): Any {
                    var index = position % item.spotsNum
                    var images = item.images
                    var imageUrl: String = ""
                    if (!images.isNullOrEmpty()) {
                        var imageUrls = images.split(",")
                        if (imageUrls.size>position-1&&!imageUrls.isNullOrEmpty()) {
            ...
            mBinding.pagerItemVenuesActivity.id = item.id.toInt()
            mBinding.tvItemVenuesPageName.text = item.name
            ...
            .OnPageChangeListener {
                ...
                override fun onPageSelected(position: Int) {
                    if (position < item.contentDataList.size) {
                        mBinding.tvItemVenuesPageName.text = "" + item.contentDataList[position].channelName
                    }else{
                        mBinding.tvItemVenuesPageName.text = item.name
                    }
                }
        ...
        if (selfLocation != null) {
            var distance = GaoDeLocation.CalculateLineDistance(
                selfLocation?.latitude.toString
                    (), selfLocation?.longitude.toString(), item.latitude.toString(), item.longitude.toString()
            )
            if (distance != "") {
...
//        }
        ...
        if (!item.contentDataList.isNullOrEmpty()) {
            valuePages.add(ValueKeyBean("活动:", "${item.contentDataList[0].title}"))
        }
...
//        }
                    ...
                    view.onNoDoubleClick {
                        if (AppUtils.isLogin()) {
                            ARouter.getInstance().build(ARouterPath.VenuesModule.SCENIC_RESERVATION_ACTIVITY)
                                .withString("scenicId", item.id.toString())
                                .navigation()
                        } else {
        ...
        mBinding.imgScenicCollect.onNoDoubleClick {
            if (onScenicLsItemClickListener != null && item.vipResourceStatus != null) {
                onScenicLsItemClickListener!!.onCollectClick(item.id.toString(), position, item.vipResourceStatus.collectionStatus)
            }
        }
    ...
    override fun payloadUpdateUi(mBinding: ItemCountryAllListBinding, position: Int, payloads: MutableList<Any>) {
