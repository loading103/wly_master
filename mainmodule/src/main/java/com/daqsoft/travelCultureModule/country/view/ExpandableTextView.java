...
import com.daqsoft.travelCultureModule.country.util.UIUtil;
...
public class ExpandableTextView extends LinearLayout implements View.OnClickListener {
    ...
    public void setOrientation(int orientation) {
        if (LinearLayout.HORIZONTAL == orientation) {
            throw new IllegalArgumentException("CountryExpandableTextView only supports Vertical Orientation.");
        }
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.CountryExpandableTextView);
        mMaxCollapsedLines = typedArray.getInt(R.styleable.CountryExpandableTextView_maxCollapsedLines, MAX_COLLAPSED_LINES);
        mAnimationDuration = typedArray.getInt(R.styleable.CountryExpandableTextView_animDuration, DEFAULT_ANIM_DURATION);
        mExpandDrawable = typedArray.getDrawable(R.styleable.CountryExpandableTextView_expandDrawable);
        mCollapseDrawable = typedArray.getDrawable(R.styleable.CountryExpandableTextView_collapseDrawable);

        textCollapse = typedArray.getString(R.styleable.CountryExpandableTextView_textCollapse);
        textExpand = typedArray.getString(R.styleable.CountryExpandableTextView_textExpand);

        if (mExpandDrawable == null) {
            mExpandDrawable = ContextCompat.getDrawable(getContext(),R.mipmap.main_arrow_down);
        }
        if (mCollapseDrawable == null) {
            mCollapseDrawable = ContextCompat.getDrawable(getContext(), R.mipmap.main_arrow_down);
        }

        if (TextUtils.isEmpty(textCollapse)) {
            textCollapse = getContext().getString(R.string.country_collapse);
        }
        if (TextUtils.isEmpty(textExpand)) {
            textExpand = getContext().getString(R.string.country_expand);
        }
        contentTextColor = typedArray.getColor(R.styleable.CountryExpandableTextView_contentTextColor, ContextCompat.getColor(getContext(), R.color.gray));
        contentTextSize = typedArray.getDimension(R.styleable.CountryExpandableTextView_contentTextSize, UIUtil.sp2px(getContext(),14));

        collapseExpandTextColor = typedArray.getColor(R.styleable.CountryExpandableTextView_collapseExpandTextColor, ContextCompat.getColor(getContext(), R.color.color_666));
        collapseExpandTextSize = typedArray.getDimension(R.styleable.CountryExpandableTextView_collapseExpandTextSize, UIUtil.sp2px(getContext(),14));

        grarity = typedArray.getInt(R.styleable.CountryExpandableTextView_collapseExpandGrarity, Gravity.CENTER);
        drawableGrarity=typedArray.getInt(R.styleable.CountryExpandableTextView_drawableGrarity, Gravity.RIGHT);
...
//            mTvContent.setMaxLines(mMaxCollapsedLines);
    ...
    private void setDrawbleAndText(){
        ...
        mTvExpandCollapse.setText(mCollapsed ? getResources().getString(R.string.country_expand) : getResources().getString(R.string.country_collapse));
    }
