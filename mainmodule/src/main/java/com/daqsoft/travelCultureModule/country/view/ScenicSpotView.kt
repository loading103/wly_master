...
import com.daqsoft.provider.bean.ScenicSpotListBean
...
class ScenicSpotView : FrameLayout {
    ...
    var scenicSpotsAdapter: CountryScenicSpotPagerAdapter? = null
    var mDatasScenicFrags: MutableList<CountryScenicSpotFragment> = mutableListOf()
    ...
    public fun setData(datas: MutableList<Spots>, fragmentManager: FragmentManager, name: String?, imageUrl: String?, scenicTags: ScenicTags?, scenicId: Int,title:String) {
        this.fragmentmanger = fragmentManager
        updateUi(datas, name, imageUrl, scenicTags, scenicId)
        binding?.tvScenicSpotDetailsAround?.text = title
    }
    ...
    private fun updateUi(datas: MutableList<Spots>, name: String?, imageUrl: String?, scenicTags: ScenicTags?, scenicId: Int) {
        ...
        if (!datas.isNullOrEmpty()) {
            ...
            for (index in 1..pageCount) {
                ...
                mDatasScenicFrags.add(CountryScenicSpotFragment.newInstance(data, name, imageUrl, "", scenicTags, scenicId))
            }
            if (pageCount == 1) {
            ...
            scenicSpotsAdapter = CountryScenicSpotPagerAdapter(mDatasScenicFrags, fragmentmanger!!)
