...
class ScenicDetailViewModel : BaseViewModel() {
     ...
     * 刷新景区数据
     */
    val refereshScenicDetail = MutableLiveData<ScenicDetailBean>()
...
//                    )
    ...
    fun getScenicBrandList(id: String) {
        ...
        MainRepository.service.getScenicBrandList("1", id, ResourceType.CONTENT_TYPE_SCENERY).excute(object : BaseObserver<HomeBranchBean>() {
...
//        }
    ...
    fun collection(resourceId: String) {
        mPresenter?.value?.loading = false
        mPresenter?.value?.isNeedRecyleView = false
        CommentRepository.service.posClloection(resourceId, ResourceType.CONTENT_TYPE_SCENERY)
    ...
    fun collectionCancel(resourceId: String) {
        mPresenter?.value?.loading = false
        mPresenter?.value?.isNeedRecyleView = false
        CommentRepository.service.posCollectionCancel(resourceId, ResourceType.CONTENT_TYPE_SCENERY)
            .excute(object : BaseObserver<Any>(mPresenter) {
                override fun onSuccess(response: BaseResponse<Any>) {
    ...
    fun thumbUp(resourceId: String) {
        CommentRepository.service.postThumbUp(resourceId, ResourceType.CONTENT_TYPE_SCENERY)
            .excute(object : BaseObserver<Any>(mPresenter) {
                override fun onSuccess(response: BaseResponse<Any>) {
    ...
    fun thumbCancell(resourceId: String) {
        CommentRepository.service.postThumbCancel(resourceId, ResourceType.CONTENT_TYPE_SCENERY)
            .excute(object : BaseObserver<Any>(mPresenter) {
    ...
    fun getOrderAddressInfo(id: String) {
        mPresenter?.value?.loading = false
        MainRepository.service.getOrderAddressInfo(ResourceType.CONTENT_TYPE_SCENERY, id)
    ...
    fun getScenicContentLs(id: String) {
        ...
        VenuesRepository.venuesService.getContentLs("publishTime", ResourceType.CONTENT_TYPE_SCENERY, id)
