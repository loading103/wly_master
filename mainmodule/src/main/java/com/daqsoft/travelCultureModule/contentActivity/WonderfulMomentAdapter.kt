    ...
    override fun setVariable(mBinding: ItemContentNewBinding, position: Int, item: ClubZixunBean) {
        ...
        if (position == 0) {
            ...
            binding = DataBindingUtil.inflate(
                ...
                R.layout.item_content_new_big_picture,
    ...
    private fun bindDataToView(binding: ItemContentNewBigPictureBinding, item: ClubZixunBean) {
        ...
        when (item.contentType) {
            ...
            "VIDEO" -> {
                ...
                GlideModuleUtil.loadDqImageWaterMark(
                    if (!item.video?.imgUrl.isNullOrEmpty()) item.video?.imgUrl else item.video?.url,
                    binding.root.context,
                    videoView.thumbImageView,
                    R.mipmap.placeholder_img_fail_240_180,
                    R.mipmap.placeholder_img_fail_240_180,
                    RequestOptions().transform(CenterCrop(), RoundedCorners(5))
                )
