package com.daqsoft.travelCultureModule.contentActivity.view
...
class GalleryTransformer : ViewPager.PageTransformer {
    ...
    fun move(page: View, position: Float) {
        ...
        //setScale
        val scaleFactor = Math.min(SCALE - Math.abs(position) * 0.2f, SCALE);
        page.setScaleX(scaleFactor);
        page.setScaleY(scaleFactor);
        val alpa = Math.min(ALPH - Math.abs(position) * 0.3f, ALPH)
