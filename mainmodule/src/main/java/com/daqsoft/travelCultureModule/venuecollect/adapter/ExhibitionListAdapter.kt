...
internal class ExhibitionListAdapter : RecyclerViewAdapter<ItemExhibitionListBinding, VenueCollectBean>(R.layout.item_exhibition_list) {

    @SuppressLint("CheckResult")
    override fun setVariable(mBinding: ItemExhibitionListBinding, position: Int, item: VenueCollectBean) {
        mBinding.datas=item
        //标签
        ...
        mBinding.imgCollect.onNoDoubleClick {
            if (AppUtils.isLogin()) {
                if (onCollectClickListener != null && item.vipResourceStatus != null) {
                    onCollectClickListener!!.onCollectClick(item.id.toString(), position, item.vipResourceStatus.collectionStatus,"CONTENT_TYPE_EXHIBIT_SHOW")
                }
            } else {
    ...
    override fun payloadUpdateUi(mBinding: ItemExhibitionListBinding, position: Int, payloads: MutableList<Any>) {
