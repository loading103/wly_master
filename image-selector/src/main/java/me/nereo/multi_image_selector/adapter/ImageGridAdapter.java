...
public class ImageGridAdapter extends BaseAdapter {
    ...
    public void setDefaultSelected(ArrayList<Image> resultList) {
        for(Image path : resultList){
            Image image = getImageByPath(path.path);
            if(image != null){
                mSelectedImages.add(image);
            }
        }
...
//            }
    ...
    class ViewHolder {
        ...
        void bindData(final Image data){
            ...
            File imageFile;
            if (!TextUtils.isEmpty(data.getThumPath())) {
                imageFile   = new File(data.getThumPath());
            }else {
                imageFile   = new File(data.path);
            }
            fileName.setText(data.name);
            fileSize.setText(readableFileSize(data.size));
            if (data.mediaType==MediaStore.Files.FileColumns.MEDIA_TYPE_AUDIO){
