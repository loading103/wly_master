...
internal class SlowLiveExpandTextView : View {
    companion object {

        private val TAG = "SlowLiveExpandTextView"
    ...
    constructor(context: Context, attr: AttributeSet?) : super(context, attr) {
        val typeArray = context.obtainStyledAttributes(attr, R.styleable.SlowLiveExpandTextView) ?: return
        val num = typeArray.indexCount
        for (i in 0 until num) {
            val attr = typeArray.getIndex(i)
            when (attr) {
                //主内容属性
                R.styleable.SlowLiveExpandTextView_slow_live_content_size -> contentTextSize = typeArray.getDimension(attr, 5f)
                R.styleable.SlowLiveExpandTextView_slow_live_content_color -> contentTextColor = typeArray.getColor(attr, Color.BLACK)
                R.styleable.SlowLiveExpandTextView_slow_live_content_bold -> contentBold = typeArray.getBoolean(attr, false)
                //特殊内容属性
                R.styleable.SlowLiveExpandTextView_slow_live_special_size -> specialTextSize = typeArray.getDimension(attr, 5f)
                R.styleable.SlowLiveExpandTextView_slow_live_special_color -> specialTextColor = typeArray.getColor(attr, Color.BLACK)
                R.styleable.SlowLiveExpandTextView_slow_live_special_bold -> specialBold = typeArray.getBoolean(attr, false)
                R.styleable.SlowLiveExpandTextView_slow_live_special_right_margin -> specialRightMargin = typeArray.getDimension(attr, 5f)
                R.styleable.SlowLiveExpandTextView_slow_live_special_horizon_click_more -> specialHorizonClickMore =
                    typeArray.getDimension(attr, 5f)
                R.styleable.SlowLiveExpandTextView_slow_live_special_vertical_click_more -> specialVerticalClickMore =
                    typeArray.getDimension(attr, 5f)
                //一些文本的设置
                R.styleable.SlowLiveExpandTextView_slow_live_current_text -> {
                    currentText = getDealStr(typeArray.getString(attr)) ?: ""
                }
                R.styleable.SlowLiveExpandTextView_slow_live_shrink_text -> {
                    shrinkText = getDealStr(typeArray.getString(attr)) ?: ""
                }
                R.styleable.SlowLiveExpandTextView_slow_live_expand_text -> {
                    expandText = getDealStr(typeArray.getString(attr)) ?: ""
                }
                R.styleable.SlowLiveExpandTextView_slow_live_expand_prifix_text -> {
                    expandPrifix = getDealStr(typeArray.getString(attr)) ?: ""
                }

                R.styleable.SlowLiveExpandTextView_slow_live_max_line -> maxLine = typeArray.getInteger(attr, Int.MAX_VALUE)
                R.styleable.SlowLiveExpandTextView_slow_live_bottom_margin -> bottomMargin = typeArray.getDimension(attr, 0f)
                R.styleable.SlowLiveExpandTextView_slow_live_special_left_margin -> {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        Log.d(TAG, "w/h:$w/$h,ow/od:$oldw/$oldh")
    }
                ...
                for (i in 0 until lineCount) {
                    ...
                    if (i == lineCount - 1) {
                        ...
                        if ((currentLineWidth + specialLeftMargin + shrinkStaticLayoutWidth) > width) {
                            ...
                            Log.d(TAG, "当前最后一行最底部距离:${height + bottomMargin}")
                        } else {
                            //都绘制在同一行
                            //绘制主内容
                            var currentWidth = 0f
                            ...
                            Log.d(TAG, "当前最后一行最底部距离:${height + shrinkStaticLayoutHeight + bottomMargin}")
                            specialRect.set(
