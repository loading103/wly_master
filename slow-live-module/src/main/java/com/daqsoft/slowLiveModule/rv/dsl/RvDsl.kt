...
internal class RvDsl(dataSet: MutableList<RvItem>) {
    internal var orientation = LinearLayoutManager.VERTICAL
    internal var reverse = false
    internal var spanCount = 1

    internal val dataSet: MutableList<RvItem> = mutableListOf()
     ...
     */
    fun item(index: Int = -1, block: () -> RvItem) {
        if (index <= -1) {
            dataSet.add(block())
        } else {
     ...
     *
     */
    fun itemDsl(index: Int = -1, block: RvItemDsl.() -> Unit) {
        val dsl = RvItemDsl()
        dsl.block()
        if (index <= -1) {
